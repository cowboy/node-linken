...
var getVersionRange = function(range) {
  // If the range is invalid, treat it as a URL.
  var parsed;
  if (!semver.valid(range)) {
    // See if range is really a URL.
    parsed = url.parse(range);
    if (parsed.protocol) {
      // If there was a hash, parse it.
      range = parsed.hash && parsed.hash.substring(1);
      // If parsed range is invalid, return '*' (match all).
      if (!semver.valid(range)) { range = '*'; }
    }
  }
  // Return the range, as-specified.
  return range;
};
